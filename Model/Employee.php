...
{
    var $table_name = 'rams.member_employeem';
    var $commute_table = 'rams.employee_commutem';
    ...
    public function employeeUpdate($params)
    {
        ...
        if (!empty($account_number)) {
            $account_update = ", account_number = hex(AES_ENCRYPT('" . $account_number . "', sha2('ramsencrypt', 512)))";
        }
    ...
    public function loadEmployee($state)
    {
        $sql = "select 
            e.user_no
            , e.user_name
    ...
    public function selectEmployee($user_no)
    {
        $sql = "select 
            m.user_id
            , m.user_name
            ...
            , m.hire_date
            , m.resign_date
            , convert(aes_decrypt(unhex(m.account_number), sha2('ramsencrypt', 512)) using utf8) as account_number
            , m.menu_group
            , c.from_time
            , c.to_time
            , c.paid_holiday
    ...
    public function commuteCheck($user_no)
    {
        $sql = "select count(0) from {$this->commute_table} where user_no = " . $user_no;
    ...
    public function commuteInsert($params)
    {
        ...
        if (!empty($user_no)) {
            $sql = "insert into {$this->commute_table} set
                  user_no = '" . $user_no . "'
                , from_time = '" . $from_time . "'
                , to_time = '" . $to_time . "'
    ...
    public function myInfoUpdate($params)
    {
        ...
        $sql = "update {$this->table_name} set 
                ...
                , certificate = '" . $certificate . "'
                , bank_name = '" . $bank_name . "'
                , account_number = hex(AES_ENCRYPT('" . $account_number . "', sha2('ramsencrypt', 512)))
                , zipcode = '" . $zipcode . "'
                , address = '" . $address . "'
                , mod_date = NOW()
                " . $pass_update . "
    ...
    public function searchId($user_name)
    {
        ...
        $sql = "SELECT user_name, user_phone, user_id FROM {$this->table_name} WHERE user_name LIKE '%" . $user_name . "%'";
        $result = $this->db->sqlRowArr($sql);
