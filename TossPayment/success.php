...
if ($payment_order_type == 'o' || $payment_order_type == 'p' || $payment_order_type == 'f' || $payment_order_type == 's') { // 물품주문, 포인트 결제, 가맹금결제, 학교 독서이력진단
...
if ($isSuccess) {
    ...
    } else if ($order_check == 's') {
        $sql = "UPDATE school_infoM SET order_num = '{$orderId}', order_method = '{$order_method}', order_state = '02', mod_date = GETDATE() WHERE school_idx = '{$_SESSION["school_idx"]}'";
        $_SESSION["order_state"] = "02";
        $_SESSION["order_state_txt"] = "결제완료";
    }
    $result = $db->execute($sql);
} else {
