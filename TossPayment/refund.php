...
if ($isSuccess) {
    ...
    cancels_lastTransactionKey = '{$responseJson->cancels->lastTransactionKey}',
    ...
    if ($cancel_amount == $ori_amount) { // 전체환불
        if ($cancelFlag == 's') {
            if ($order_check == 'i' || $order_check == 'f' || $order_check == 'p' || $order_check == 's') {
                $order_state = '99';
    ...
    if ($order_check == 'i') { // 원비
        ...
        $sql = "UPDATE school_infoM SET order_state = '{$order_state}', mod_date = GETDATE() WHERE order_num = '{$responseJson->orderId}'";
    }
    $result = $db->execute($sql);
} else {
