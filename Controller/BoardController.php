...
class BoardController extends Controller
{
            ...
            if (!empty($result)) {
                foreach ($result as $key => $val) {
                    if (!empty($result[$key]['files'])) {
                        $result[$key]['file_link'] = "<a class=\"btn btn-outline-primary small\" href=\"/files/notice_file/" . $val['files'] . "\" download><i class=\"fa-solid fa-file-arrow-down\"></i> 다운로드</a>";
                    } else {
    ...
    public function fileBoardInsert($request)
    {
        ...
        try {
            if (empty($title) || empty($category1) || empty($contents)) {
                throw new Exception('필수값이 누락되었습니다.', 701);
            }
    ...
    public function fileBoardUpdate($request)
    {
        ...
        try {
            if (empty($board_idx) || empty($title) || empty($category1) || empty($contents)) {
                throw new Exception('필수값이 누락되었습니다.', 701);
            }
        ...
        try {
            ...
            if (!empty($result)) {
                foreach ($result as $key => $val) {
                    ...
                    if (!empty($result[$key]['files'])) {
                        $result[$key]['file_link'] = "<a class=\"btn btn-outline-primary small\" href=\"/files/board_file/" . $val['files'] . "\" download><i class=\"fa-solid fa-file-arrow-down\"></i> 다운로드</a>";
                    } else {
                        $result[$key]['file_link'] = '';
                    }
                }
