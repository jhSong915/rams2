...
include_once $_SERVER['DOCUMENT_ROOT'] . "/Model/BoardTip.php";
...
class BoardTipController extends Controller
{
        ...
        try {
            ...
            if (!empty($result_notice)) {
                foreach ($result_notice as $key => $val) {
                    ...
                    if (!empty($val['file_name'])) {
                        $down_icon = "<span class=\"badge rounded-pill bg-success ms-2\"><i class=\"fa-regular fa-file-zipper\"></i></span>";
                    } else {
                        $down_icon = "";
                    }
                    ...
                    $title = "[공지] " . $head . $val['title']
                        ...
                        . $val['cnt1'] . "<i class=\"fa-regular fa-thumbs-up ms-1 me-1\"></i>" . $val['cnt2'] . "</span>" . $down_icon;


                    $notice .= "<tr class=\"table-warning tc align-middle text-center\">
                                  <td>" . $val['board_idx'] . "</td>
            ...
            if (!empty($result_board)) {
                foreach ($result_board as $key => $val) {
                    ...
                    $board .= "<tr class=\"tc align-middle text-center\">
                                   <td>" . $val['board_idx'] . "</td>
                                   <td>" . $notice_head . $head . $title . "</td>
                                   <td>" . $writer . "</td>
                                   <td>" . $val['reg_date'] . "</td>
                               </tr>";
                }
        ...
        try {
            ...
            if (!empty($result)) {
                foreach ($result as $key => $val) {
                    ...
                    if ($params['writer_no'] == $val['user_no']) {
    ...
    public function cmtLikeChange($request)
    {
        ...
        try {
            ...
            if (!empty($like_check)) {
                $comment_idx = !empty($like_check['comment_idx']) ? $like_check['comment_idx'] : '';

                $result = $this->boardTipInfo->likeDelete($comment_idx);
            } else {
                $params = array(
                    "board_idx" => !empty($board_idx) ? $board_idx : '',
                    "user_no" => !empty($user_no) ? $user_no : '',
