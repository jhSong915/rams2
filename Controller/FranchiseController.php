...
include $_SERVER['DOCUMENT_ROOT'] . "/Model/Franchisee.php";
...
class FranchiseController extends Controller
{
    ...
    public function centerInsert($request)
    {
        ...
        try {
            if (empty($center_name) || empty($owner_name) || empty($address) || empty($tel_num) || empty($location)) {
                throw new Exception('잘못된 접근입니다. (필수 값 누락)', 600);
            }
    ...
    public function centerUpdate($request)
    {
        ...
        try {
            if (empty($franchise_idx) || empty($center_name) || empty($owner_name) || empty($address) || empty($tel_num) || empty($location)) {
                throw new Exception('잘못된 접근입니다. (필수 값 누락)', 600);
            }
