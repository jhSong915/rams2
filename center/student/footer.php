...
$infomanage_data2 = $permissionCls->selectInfomanage('1');
...
if (!empty($infomanage_data)) {
    $footer_array = array();
    if (!empty($infomanage_data['center_name'])) {
        array_push($footer_array, "<span>책읽기와 글쓰기 리딩엠 " . $infomanage_data['center_name'] . "</span>");
    }
    if (!empty($infomanage_data['address'])) {
        array_push($footer_array, "<span>" . $infomanage_data['address'] . "</span>");
    }
    if (!empty($infomanage_data['tel_num'])) {
        array_push($footer_array, "<span><a href=\"tel:" . htmlspecialchars(phoneFormat($infomanage_data['tel_num'], true)) . "\">Tel : " . htmlspecialchars(phoneFormat($infomanage_data['tel_num'], true)) . "</a></span>");

    }
    if (!empty($infomanage_data['fax_num'])) {
        array_push($footer_array, "<span>FAX &#58; " . phoneFormat($infomanage_data['fax_num'], true) . "</span>");
    }
    if (!empty($infomanage_data['biz_no'])) {
        array_push($footer_array, "<span>사업자등록번호 &#58; " . $infomanage_data['biz_no'] . "</span>");
    }
    if (!empty($infomanage_data['center_no'])) {
        array_push($footer_array, "<span>학원등록번호 &#58; " . $infomanage_data['center_no'] . "</span>");
    }

    $footer_array2 = array(
        "<span>본사대표 &#58; " . $infomanage_data2['owner_name'] . "</span>",
        "<span>사업자등록번호 &#58; " . $infomanage_data2['biz_no'] . "</span>",
...
<div class="container-fluid p-4 bg-dark mt-auto align-items-end">
    <div class="d-flex align-items-center justify-content-between text-white small">
        <div class="text-muted">&copy; 리딩엠 <?= date('Y') ?></div>
        <div class="mb-2">
