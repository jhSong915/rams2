...
<div class="row mt-2 mb-2">
    ...
    <div class="col-12 mt-2">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col align-self-center">나의수업목록</div>
                    <div class="col-auto"><input type="date" id="dtMyLessonDate" class="form-control" value="<?= date('Y-m-d') ?>" /></div>
                </div>
            </div>
            <div class="card-body">
                <table id="CenterLessonMain" class="table table-sm table-bordered table-hover">
                    <thead>
                        <th width="10%">종류</th>
                        <th width="10%">날짜</th>
                        <th width="15%">수업시간</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#dtMyLessonDate").change(function () {
                loadLessonMain();
            });
        });

        function loadLessonMain() {
            var lesson_date = $("#dtMyLessonDate").val();
            if (!lesson_date) {
                alert("수업날짜를 선택해주세요.");
                return false;
            }
            $.ajax({
                ...
                data: {
                    ...
                    teacher_idx: '<?= $_SESSION['logged_no'] ?>',
                    lesson_date: lesson_date,
                },
                success: function(result) {
                    if (result.success && result.data) {
                        $('#CenterLessonMain').DataTable({
                            autoWidth: false,
                            destroy: true,
                            data: result.data,
                            stripeClasses: [],
                        ...
                        $("#CenterLessonMain").DataTable().destroy();
                        $("#CenterLessonMain > tbody").empty();
                    }
                    return false;
                },
