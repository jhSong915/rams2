...
<script>
    $("#btnFranchiseFeePay").click(function() {
        ...
        var compare_date = new Date(order_ym + "-05");
        compare_date.setMonth(compare_date.getMonth() + 1);
        today_date = moment(today_date).format('YYYY-MM-DD');
        compare_date = moment(compare_date).format('YYYY-MM-DD');
        if (today_date < compare_date) {
            alert("계속가맹금 결제는 년월을 기준으로 다음 달 5일부터 가능합니다.\n결제가능일 : " + compare_date + "부터 가능");
            return false;
        }
