...
class ReportController extends Controller
{
    ...
    public function reportSettingInsert($request)
    {
        ...
        try {
            ...
            $check_data = $this->reportInfo->reportSettingLoad($franchise_idx, $user_no);
    ...
    //보고서 양식 로드
    public function reportSettingLoad($request)
    {
        $franchise_idx = !empty($request['franchise_idx']) ? $request['franchise_idx'] : '';
        $user_no = !empty($request['user_no']) ? $request['user_no'] : '';
        try {
            ...
            $result = $this->reportInfo->reportSettingLoad($franchise_idx, $user_no);
            return $result;
        } catch (Exception $e) {
            return $this->getMsgException($e);
        }
    ...
    public function reportSettingPageLoad($franchise_idx, $user_no)
    {
        try {
            if (empty($franchise_idx) || empty($user_no)) {
                throw new Exception('필수값이 누락되었습니다.', 701);
    ...
    //이전 작성내용 로드
    public function reportContentsLoad($request)
    {
        $franchise_idx = !empty($request['franchise_idx']) ? $request['franchise_idx'] : '';
        $user_no = !empty($request['user_no']) ? $request['user_no'] : '';
        ...
        try {
            ...
            $result = $this->reportInfo->reportContentsLoad($franchise_idx, $user_no);
    ...
    //이전 작성내용 로드
    public function reportContentsPageLoad($franchise_idx, $user_no)
    {
        try {
            if (empty($franchise_idx) || empty($user_no)) {
                throw new Exception('필수값이 누락되었습니다.', 701);
    ...
    public function selectReportData($user_no, $franchise_idx, $report_idx)
    {
        try {
            ...
            $result = $this->reportInfo->selectReportData($user_no, $franchise_idx, $report_idx);
