...
class StudentFeeController extends Controller
{
    ...
    public function getStudentMemo($request)
    {
        $center_idx = !empty($request['center_idx']) ? $request['center_idx'] : '';
        $student_idx = !empty($request['student_idx']) ? $request['student_idx'] : '';

        try {
            if (empty($center_idx) || empty($student_idx)) {
                throw new Exception('필수값이 누락되었습니다.', 701);
            }
            $result = $this->studentFeeInfo->getStudentMemo($center_idx, $student_idx);
            if (!empty($result)) {
                return $result;
            } else {
                throw new Exception('학생정보를 확인할 수 없습니다.', 701);
            }
        } catch (Exception $e) {
            return $this->getMsgException($e);
        }
