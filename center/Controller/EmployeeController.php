...
class EmployeeController extends Controller
{
    ...
    public function employeeUpdate($request)
    {
        ...
        try {
            if (
                empty($franchise_idx) || empty($user_no) || empty($user_name) || empty($user_phone) || empty($emergencyhp) 
                || empty($birth) || empty($gender) || empty($state) || empty($position) || empty($email) || empty($zipcode)
                 || empty($address) || empty($paid_holiday) || empty($unpaid_holiday) || empty($from_time) || empty($to_time)
            ) {
