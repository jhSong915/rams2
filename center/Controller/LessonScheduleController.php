...
class LessonScheduleContoller extends Controller
{
    ...
    public function loadLessonMain($request)
    {
        ...
        try {
            if (empty($center_idx) || empty($teacher_idx)) {
                throw new Exception('필수값이 누락되었습니다.', 701);
            }
            $result = $this->lessonScheduleInfo->loadLessonMain($center_idx, $teacher_idx, $lesson_date);
            if (!empty($result)) {
                foreach ($result as $key => $val) {
                    $result[$key]['supple_yn'] == 'Y' ? '(보강)' : '';
                    $result[$key]['lesson_type'] = $result[$key]['lesson_type'] . $result[$key]['supple_yn'];
                }
            }
            return $result;
        } catch (Exception $e) {
