...
if (!empty($_SESSION['logged_no'])) {
    $myMenu = $permissionCmp->myMenuList($_SESSION['logged_no'], 'center');
    $myMenuList = explode(",", $myMenu);
    $menuList = $menuClassCmp->getMenu('center');
    ...
    $manage_arr1 = array(21, 22, 23);  //코드관리, 정보관리, 수납항목관리
    $manage_arr2 = array(24);  // 원생목록
    $manage_arr3 = array(25, 26, 27, 28, 29);  // 직원목록, 출근부확인, 근무평가, 직원교육, 업무보고
    $manage_arr4 = array(30);  // 상담관리
    $manage_arr5 = array(31);  // 커리큘럼
    $manage_arr6 = array(32, 33);  // 문자, 문자발송내역
    $manage_arr7 = array(34);  // 물품주문
    $manage_arr8 = array(35, 36, 37, 38);  // 물품주문, 원비수납, 원비환불, 매출확정, 교사매출
}
?>
<ul id="menuList" class="nav nav-tabs">
        ...
        <ul class="dropdown-menu">
            ...
            foreach ($printMenu as $printKey => $printList) {
                if (in_array($printKey, $manage_arr1)) {
                    echo "<li><a class='dropdown-item menu' href='" . $printList['link'] . "'>" . $printList['menu_name'] . "</a></li>";
                }
            }
            ?>
    ...
    <li class="nav-item">
        ...
        foreach ($printMenu as $printKey => $printList) {
            if (in_array($printKey, $manage_arr4)) {
                echo "<a class='nav-link menu' href='" . $printList['link'] . "'>" . $printList['menu_name'] . "</a>";
            }
        }
        ?>
    </li>
    ...
    <li class="nav-item dropdown">
        <button type="button" class="nav-link dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">문자</button>
        <ul class="dropdown-menu">
            <?php
            foreach ($printMenu as $printKey => $printList) {
                if (in_array($printKey, $manage_arr6)) {
                    echo "<li><a class='dropdown-item menu' href='" . $printList['link'] . "'>" . $printList['menu_name'] . "</a></li>";
                }
            }
            ?>
    ...
    <li class="nav-item dropdown">
        ...
        <ul class="dropdown-menu">
            ...
            <?php
            foreach ($printMenu as $printKey => $printList) {
                if (in_array($printKey, $manage_arr7)) {
                    echo "<li><a class='dropdown-item menu' href='" . $printList['link'] . "'>" . $printList['menu_name'] . "</a></li>";
                }
            }
            ?>
    ...
    <li class="nav-item dropdown">
        ...
        <ul class="dropdown-menu">
            <?php
            foreach ($printMenu as $printKey => $printList) {
                if (in_array($printKey, $manage_arr8)) {
                    echo "<li><a class='dropdown-item menu' href='" . $printList['link'] . "'>" . $printList['menu_name'] . "</a></li>";
                }
            }
            ?>
