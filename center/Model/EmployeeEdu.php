<?php
include_once $_SERVER['DOCUMENT_ROOT'] . "/center/Model/Model.php";
...
class EmployeeEduInfo extends Model
{
    ...
    {
        $sql = "SELECT edu_idx, edu_name, edu_type, edu_target, edu_way FROM {$this->table_name} WHERE edu_idx = '" . $edu_idx . "'";
    ...
    {
        $sql = "SELECT user_no, user_name FROM member_centerm WHERE franchise_idx = '" . $franchise_idx . "' AND user_id <> 'admin' AND state = '00'";
    ...
    public function eduCenterScheduleSelect($franchise_idx)
    {
        $sql = "SELECT 
                ...
                LEFT OUTER JOIN {$this->table_name} e 
