...
class LessonScheduleInfo extends Model
{
    ...
    {
        $sql = "exec sp_schedule_holdover_week '" . $franchise_idx . "','" . $startweek1 . "','" . $endweek1 . "','" . $startweek2 . "','" . $endweek2 . "'";
        $result = $this->db->execute($sql);

        return $result;
    }

    public function loadLessonMain($center_idx, $teacher_idx, $lesson_date)
    {
        $sql = "SELECT C.code_name AS lesson_type, M.supple_yn, M.lesson_date, CONCAT(M.lesson_fromtime, ' ~ ',M.lesson_totime) AS lesson_time, ISNULL(C2.code_name, '') AS grade
                , ISNULL(B.book_name, '') AS lesson_book
                , ISNULL((SELECT TOP(1) B2.book_name FROM lessonM M2 LEFT OUTER JOIN bookM B2 ON B2.book_idx = M2.lesson_book_idx 
                    WHERE M2.franchise_idx = '{$center_idx}' AND M2.teacher_idx = '{$teacher_idx}' AND M2.lesson_date > '{$lesson_date}'), '') AS next_lesson_book
                , COUNT(T.student_idx) AS std_cnt, COUNT(AT.student_idx) AS attend_cnt
                FROM {$this->table_name} M 
                LEFT OUTER JOIN {$this->lesson_table_name} T ON M.schedule_idx = T.schedule_idx
                LEFT OUTER JOIN lesson_attendT AT ON M.schedule_idx = AT.schedule_idx AND AT.attend_Yn = 'N'
                LEFT OUTER JOIN codeM C ON C.code_num2 = M.lesson_type AND C.code_num1 = '04'
                LEFT OUTER JOIN codeM C2 ON C2.code_num2 = M.lesson_grade AND C2.code_num1 = '02'
                LEFT OUTER JOIN bookM B ON B.book_idx = M.lesson_book_idx
                WHERE M.franchise_idx = '{$center_idx}' AND M.teacher_idx = '{$teacher_idx}' AND lesson_date = '{$lesson_date}'
                GROUP BY C.code_name, M.lesson_date, M.lesson_fromtime, M.lesson_totime, C2.code_name, B.book_name,M.lesson_book_idx, M.supple_yn
                ORDER BY M.lesson_fromtime";
        $result = $this->db->sqlRowArr($sql);
        return $result;
    }
}
