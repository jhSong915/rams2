...
class StudentFeeInfo extends Model
{
    ...
    {
        $smonth = date('Y-m-d', strtotime($paymonth . '-01'));
        $emonth = date('Y-m-t', strtotime($paymonth));
        ...
        LEFT OUTER JOIN lesson_attendT LAT ON LM.schedule_idx = LAT.schedule_idx
        ...
        WHERE LM.franchise_idx = '{$franchise_idx}' AND LT.student_idx = '{$student_idx}' AND LM.lesson_date BETWEEN '{$smonth}' AND '{$emonth}' AND LAT.attend_yn = 'Y'
    ...
    {
        $year = mb_split("-", $paymonth);
        $from_date = $year[0] . "-01";
        $to_date = $year[0] . "-12";

        $sql = "SELECT I.order_ym, I.order_date, SUM(I.order_money - I.refund_money) AS amount, C.code_name, C2.code_name AS order_state, MC.user_name, I.pay_memo FROM invoiceM I
        ...
        LEFT OUTER JOIN codeM C2 ON I.order_state = C2.code_num2 AND C2.code_num1 = '45'
        WHERE I.franchise_idx = '{$center_idx}' AND I.student_idx = '{$student_idx}' AND I.order_ym BETWEEN '{$from_date}' AND '{$to_date}' AND I.order_state IN ('01', '02', '03', '04')
        GROUP BY I.order_ym, I.order_date, C.code_name, C2.code_name, MC.user_name, I.pay_memo
        ORDER BY I.order_ym DESC";
        $result = $this->db->sqlRowArr($sql);
    ...
    public function getStudentMemo($center_idx, $student_idx)
    {
        $sql = "SELECT user_memo FROM member_studentM WHERE franchise_idx = '{$center_idx}' AND user_no = '{$student_idx}'";
        $result = $this->db->sqlRow($sql);
        return $result;
    }
}
