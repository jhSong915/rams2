...
class ReportInfo extends Model
{
    ...
    public function reportSettingUpdate($params)
    {
        ...
        $sql = "UPDATE {$this->setting_table_name} SET
                  mod_date = getdate()
                " . $title_qry . "
                WHERE franchise_idx = '1' AND user_no = '" . $params['user_no'] . "'";
    ...
    public function reportContentsLoad($user_no)
    {
        $sql = "SELECT TOP (1)
                ...
                WHERE user_no = '" . $user_no . "' AND franchise_idx = '1' ORDER BY reg_date DESC";
        $result = $this->db->sqlRow($sql);
    ...
    public function selectReportData($user_no, $franchise_idx, $report_idx)
    {
        ...
        , content1 , content2 , content3 , content4 , content5 , content6 , content7 , content8 , content9 , content10
        FROM {$this->report_table_name}
        WHERE user_no = '" . $user_no . "' AND report_idx = '" . $report_idx . "' AND franchise_idx = '{$franchise_idx}'";
        ...
        , content1 , content2 , content3 , content4 , content5 , content6 , content7 , content8 , content9 , content10
        FROM {$this->report_table_name}
        WHERE user_no = '" . $user_no . "' AND report_idx < '" . $report_idx . "' AND franchise_idx = '{$franchise_idx}' ORDER BY reg_date DESC";
