$(document).ready(function () {
    ...
    $("#bannerList").on("click", ".tc", function (e) {
        bannerSelect(e);
    });
            ...
            if (result.success && result.data) {
                $('#dataTable').DataTable({
                    ...
                    createdRow: function (row, data) {
                        ...
                        $(row).attr('data-banner_idx', data.banner_idx);
...
function bannerSelect(e) {
    ...
    var banner_idx = $(e.target).parents("tr").data('banner_idx');
    if ($(e.target).parent('.bg-light').length) {
        $(".tc").removeClass('bg-light');
        $("#txtBannerLink").val('');
        $("#dtFromDate").val('');
        $("#dtToDate").val('');
        $("#chkUseYn").prop('checked', false);
        $("#chkMainYn").prop('checked', false);
        $("#txtOrders").val('');
        $('#txtImageName').val('');
    } else {
        $(".tc").removeClass('bg-light');
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    if (result.data) {
                        $("#txtBannerLink").val(result.data.banner_link);
                        $("#dtFromDate").val(result.data.from_date);
                        $("#dtToDate").val(result.data.to_date);
                        $("#chkUseYn").prop('checked', false);
                        $("#chkMainYn").prop('checked', false);
                        if (result.data.mainYn == 'Y') {
                            $("#chkMainYn").prop('checked', true);
                        } else {
                            $("#chkMainYn").prop('checked', false);
                        }
                        if (result.data.banner_visible == 'Y') {
                            $("#chkUseYn").prop('checked', true);
                        } else {
                            $("#chkUseYn").prop('checked', false);
                        }
                        $("#txtOrders").val(result.data.orders);
                        // $('#txtImageName').val(result.data.banner_image);
