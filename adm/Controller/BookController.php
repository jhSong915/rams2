...
class BookController extends Controller
{
    ...
    public function getBookInfo($request)
    {
        $book_isbn = !empty($request['book_isbn']) ? $request['book_isbn'] : '';
        try {
            if (empty($book_isbn)) {
            ...
            if (!empty($book_isbn) && strlen($book_isbn) >= 10) {
                ...
                $dt = json_decode($res, true);
                if (!empty($dt['documents'][0]['title'])) {
                    $result['title'] = $dt['documents'][0]['title'];
                }
                if (!empty($dt['documents'][0]['authors'])) {
                    $result['authors'] = $dt['documents'][0]['authors'];
                }
                if (!empty($dt['documents'][0]['publisher'])) {
                    $result['publisher'] = $dt['documents'][0]['publisher'];
                }
            }
            return $result;
        } catch (Exception $e) {
