...
$USER_AGENT = 'example.com kakaowork bot';
# 전송 헤더 설정
$aHeader    = array();
...
curl_setopt($ch, CURLOPT_HTTPHEADER, $aHeader); // 요청 헤더
curl_setopt($ch, CURL_HTTP_VERSION_1_1, 1); // HTTP 1.1 사용 여부
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1); // 서버 응답에 대한 설정 Status Code 300 ~ 308
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // 반환 설정
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // SSL 인증서 유효성 검사
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false); // SSL 검증 설정 (호스트)
curl_setopt($ch, CURLOPT_SSLVERSION, 6); // SSL 버전 https 통신 시 필요
curl_setopt($ch, CURLOPT_HEADER, 1); // 헤더 값 출력 여부
curl_setopt($ch, CURLOPT_VERBOSE, 0); // 자세한 정보 출력 여부
if ($json_data) {
